...
FinPrisonData <- read.spss("C:/Users/benny_000/Dropbox/AAAKTUELLT/MI/recidivists and nonrecidivsts all cases.sav", 
...
### Behövs inte - åtminstone i det här skedet.
# ###  Create Variable for current drug problems
# 
# UDw7m4mod<-'UDrug =~ Drug2+Drug5+Drug6+Drug8+Drug7'
# UDw7m4cfa<-cfa(UDw7m4mod, UsedDrugs, std.lv = T, estimator = "WLSMV")
# reliability(UDw7m4cfa)
# drugprob<-predict(UDw7m4cfa)
# table(drugprob)
# DrugCurrent<-ifelse(drugprob > 0.05,2,ifelse(drugprob > -1.30,1,0))
# table(DrugCurrent)
# 
# UsedDrugs<-data.frame(UsedDrugs, DrugCurrent)
# NoDrugs<-data.frame(NoDrugs, rep(0,935))
# names(NoDrugs)[64]<-"DrugCurrent"
# names(UsedDrugs)[64]<-"DrugCurrent"
# subsetData2<-rbind(UsedDrugs,NoDrugs)
# 
# summary(subsetData2)
# subsetData2[64] <- lapply(subsetData2[64], ordered)
# 
# 
# summary(as.integer(FinPrisonData$ps.prisonTerms))
# 
# 
